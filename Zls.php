...
class Z
{
    ...
    public static function safePath($path, $prefix = '~APP~', $entr = false)
    {
        ...
        $siteRoot = is_bool($entr) ? self::path('.', false, $entr) : $entr;
    ...
    public static function dump()
    {
        ...
        $html = @ob_get_clean();
        $beautify = (!Z::isCli() || Z::isResidentMemory()) && !$isXdebug && !self::isAjax();
        if ($beautify) {
    ...
    public static function isResidentMemory()
    {
        static $yes;
        if (!$yes) {
    ...
    {
        return Zls_Context_Default::get($id, $def);
    }
    /**
     * 获取数组的值
     *
    ...
    {
        Zls_Context_Default::set($id, $data);
    }
    /**
     * 获取当前UNIX毫秒时间戳
     * @return float
     ...
     */
    function &config($configName = null, $caching = true, $default = null) {
        if (empty($configName)) {
     ...
     * 字符串截取
     */
    public static function strLenSub($str, $len, $encoding = 'utf8')
    {
    ...
    public static function strCamel2Snake($str, $delimiter = '_')
    {
        ...
        return is_array($str) ? (function () use ($to, $str) {
            ...
            foreach ($str as $key => $v) {
                if (is_array($v)) {
                    $arr[$to($key)] = self::strCamel2Snake($v);
                } else {
                    $arr[$to($key)] = $v;
                }
            }
    ...
    {
        $config = self::config();
        $onCompatible = $config->getRouterCompatible();
        ...
        self::throwIf(!($controllerObject instanceof Zls_Controller), 500, '[ ' . $class . ' ] not a valid Zls_Controller', 'ERROR');
        ...
        if ($method) {
            ...
            $view = function ($contents) use ($controllerShort, $method) {
                if (is_array($contents)) {
                    $contents = self::view()->set($contents)->load("$controllerShort/$method", [], true);
                }
                return $contents;
            };
            ...
            $exist = method_exists($controllerObject, $methodFull);
            if (!$exist && $onCompatible) {
                $methodFull = self::strSnake2Camel($methodFull);
                $exist = method_exists($controllerObject, $methodFull);
            }
            if (!$exist) {
                $containCall = method_exists($controllerObject, 'call');
                self::throwIf(!$containCall, 404, 'Method' . ($requestMethod ? "({$requestMethod})" : '') . ' [ ' . $class . '->' . $methodFull . '() ] not found');
     ...
     */
    public static function getOpt($key = null, $default = null, $hasGNU = true)
    {
        if (!self::isCli()) {
            return null;
        }
        ...
        return self::tap($result, function (&$result) use ($key, $hasGNU, $default) {
            if (!is_null($key)) {
    ...
    {
        if (!self::isPost()) {
            return self::postText($key, $default, $xssClean);
        }
        $data = self::getGlobalData(ZLS_PREFIX . 'post', []);
    ...
    public static function postText($key = null, $default = null, $xssClean = true)
    {
        ...
        if ($raw = self::postRaw()) {
            ...
            $input = self::getGlobalData(ZLS_PREFIX . 'post', []);
        }
        if (is_null($key)) {
            return $input;
        }
    ...
    {
        if (!self::isResidentMemory()) {
            return file_get_contents('php://input');
        }
        return self::server('ZLS_POSTRAW');
    }
    /**
     * 设置数组
     */
    ...
    public static function sessionStart($id = null)
    {
        if (!!$id || !self::di()->has('ZlsSessionID')) {
            ...
            if (!self::isCli()) {
                ...
                self::throwIf(!$sessionHandle, 500, 'residentMemory mode must set the SessionHandle');
                if (self::isResidentMemory()) {
                    $sessionHandle->residentMemory($sessionId);
                }
            }
            if ($sessionId) {
    ...
    {
        return Zls_Context_Default::has($id);
    }
    /**
     * 设置session配置
     * @param null $key
    ...
    {
        self::throwIf(self::isCli() && !Z::isResidentMemory(), 500, 'urlPath() can not be used in cli mode');
    ...
    public static function checkData($data = [], $rules = [], &$returnData = null, &$errorMessage = '', &$errorKey = null, &$db = null)
    {
        ...
        foreach ($rules as $key => $keyRules) {
            foreach ($keyRules as $rule => $message) {
                ...
                $db = $db instanceof \Zls_Database_ActiveRecord ? $db : Z::db();
                $break = false;
                $returnValue = null;
                $isOkay = false;
                if ('function' == $_r) {
                ...
                if (!is_null($returnValue)) {
                    $data[$key] = $returnData[$key] = $returnValue;
                }
                if ($break) {
                    break;
                }
     ...
     */
    function &db($group = '', $isNewInstance = false) {
    ...
    {
        if (self::config()->runState && (self::isResidentMemory() || (!ZLS_RUN_MODE_PLUGIN && !self::isCli()))) {
...
class Zls
{
    ...
    public static function run()
    {
        ...
        if (!in_array($config->getRequest()->getPathInfo(), ['/favicon.ico'], true)) {
            $level = $config->getExceptionLevel();
            error_reporting($level);
            @ini_set('display_errors', $level > 0);
    ...
    private static function runCli()
    {
        ...
        $isTask = in_array($current, ['task', '-task'], true);
        $activity = $isTask ? str_replace('/', '_', Z::getOpt('task')) : Z::arrayGet($args, 1);
        ...
        try {
            if ($isTask) {
                ...
                try {
                    ...
                    $taskObject = z::factory($taskName . 'Task', true);
                }
    ...
    public static function runWeb()
    {
        ...
        if ((bool) $_apiDoc) {
            ...
            if ('self' == $api) {
                $docComment::html(
                    'self',
                    $docComment::apiMethods($docComment::getClassName($class), $method, true, $getHmvcModuleName)
                );
            } else {
                if ('all' == $api) {
                    ...
                    $docComment::html(
                        'parent',
                        $docComment::docComment($docComment::getClassName($_route->getController()), $getHmvcModuleName)
                    );
                }
            }
            return '';
...
class Zls_Command
{
    ...
    public function __construct($args)
    {
        ...
        try {
            try {
                $taskObject = Z::factory($commandName . 'Command', true);
            } catch (\Zls_Exception_500$e) {
    ...
    abstract class Zls_Bean
{
        ...
        public function __invoke(...$key)
    {
        ...
        final public function toArray($fields = [], $containExte = true)
    {
            $selfVars = get_object_vars($this);
            $selfVarKeys = array_keys($selfVars);
            $fieldFilter = [];
            $args = $selfVars;
            if ($fields && is_array($fields)) {
                $args = Z::arrayFilter($args, function ($_, $key) use ($fields, &$fieldFilter) {
            ...
            foreach ($methods as $method) {
                if ($key = static::_get($method)) {
                    if (in_array($key, $fieldFilter, true)) {
                        continue;
                    }
                    if (in_array($key, $selfVarKeys)) {
    ...
    abstract class Zls_Task
{
        ...
        public function _execute($args)
    {
        ...
        final protected function _log($msg, $time = true)
    {
            if ($this->debug) {
                $nowTime = '' . Z::microtime();
        ...
        final protected function wrapError($args)
    {
            try {
                $this->execute($args);
            } catch (\Exception$e) {
    ...
    {
            echo 'Execute failed, exited with error: ' . $err->getMessage() . ' ' . PHP_EOL;
        }
    ...
    class Zls_Router_PathInfo extends Zls_Router
{
    ...
    {
            $config = Zls::getConfig();
            $uri = trim($config->getRequest()->getPathInfo(), '/');
            if (!$uri && $config->getRouterCompatible() && ($uri = Z::get('_s', ''))) {
                $data = Z::get();
                unset($data['_s']);
                Z::setGlobalData(ZLS_PREFIX . 'get', $data);
            }
        ...
        public function url($action = '', $getData = [], $opt = ['subfix' => true, 'ishmvc' => false])
    {
            ...
            if ($isPathinfo) {
                ...
                $url = $root ? $index . '?_s=' . $action : '/' . $index . '?_s=' . $action;
            }
            $url = rtrim($url, '/');
    ...
    abstract class Zls_Task_Multiple extends Zls_Task
{
        public function _execute($args)
    {
        ...
        private function getCurrentPids($lockFilePath)
    {
            if (file_exists($lockFilePath)) {
                return explode("\n", file_get_contents($lockFilePath));
            }
...

{
    ...
    {
            $isCli = Z::isCli() && !Z::isResidentMemory();
            if (is_null($isJson)) {
                $isJson = Z::isAjax();
            }
            if ($isJson) {
    ...
    class Zls_Route
{
    ...
    {
            $this->method = Z::config()->getMethodPrefix() . $method;
            return $this;
        }
 ...
 * @method Zls_Config setTraceStatus($e) 开启调试日志
 ...
 * @method Zls_Config setBootstrapHandle(callable $e)
 ...
 * @method Zls_Config setRouterCompatible(boolean $on) 路由兼容模式
 ...
 * @method callablgetBootstrapHandle()
 ...
 * @method bool|string getRouterCompatible()
 */
    class Zls_Config
{
        ...
        private $exceptionLevel = E_ALL ^ E_DEPRECATED;
    ...
    {
            return $this->maintainIpWhitelist ?: $this->getSysConfig('ipWhitelist', []);
        }
        /**
        ...
        public function setMaintainIpWhitelist(array $maintainIpWhitelist)
    {
        ...
        public function getSysConfig($key, $default = '')
    {
            static $zls = null;
            if (is_null($zls)) {
                $zls = Z::config()->find('zls') ? Z::config('zls') : [];
            }
            if (!$key) {
                return $zls;
            }
            return Z::arrayGet($zls, $key, $default);
        }
        /**
         * 按照包的顺序查找配置文件
         *
    ...
    {
            return $this->showError ?: $this->getSysConfig('showError');
        }
        /**
         * 设置别名
         *
        ...
        public function getExceptionJsonRender()
    {
            if (!$this->exceptionJsonRender) {
                $this->exceptionJsonRender = function (\Zls_Exception$e) {
    ...
    {
            return $this->isMaintainMode ?: $this->getSysConfig('maintainMode');
        }
        /**
    ...
    class Zls_Logger_Dispatcher
{
        ...
        final public static function dispatch(Zls_Exception $exception, $result = false)
    {
            ...
            } elseif (Z::isCli() && !Z::isResidentMemory()) {
    ...
    class Zls_Cache_File implements Zls_Cache
{
        ...
        private function pack($userData, $cacheTime)
    {
        ...
        function &instance($key = null, $isRead = true) {
            return $this;
        }
        public function reset()
    {
